...
public class MatrixMultiplicationGPU extends Kernel implements MatrixMultiplication {
	 ...
	 */
	public MatrixMultiplicationGPU(float[] matrixA, int matrixAWidth, int matrixAHeight, boolean transposeMatrixA,
	...
	private void computeMatrixColumn(int resultColumnIndex) {
		...
		for (int lineInA = 0; lineInA < this.matrixAHeight; lineInA++) {
			...
			for (int columnInB = 0; columnInB < this.matrixBHeight; columnInB++) {
				...
				float a = get(matrixA, matrixAWidth, matrixAHeight, columnInB, lineInA, isMatrixATranspose == 1);
				float b = get(matrixB, matrixBWidth, matrixBHeight, resultColumnIndex, columnInB, isMatrixBTranspose == 1);
				value += a * b;
			}
