...
public class MatrixMultiplicationDoublePrecisionGPUv2 extends Kernel implements MatrixMultiplicationDoublePrecision {
	 ...
	 */
	public MatrixMultiplicationDoublePrecisionGPUv2(double[] matrixA, int matrixAWidth, int matrixAHeight, boolean transposeMatrixA,
	...
	private void computeMatrixColumn(int col, int row) {
		...
		for (int columnInB = 0; columnInB < this.matrixBHeight; columnInB++) {
			...
			// GPU
			double a = get(matrixA, matrixAWidth, matrixAHeight, columnInB, row, isMatrixATranspose == 1);
			double b = get(matrixB, matrixBWidth, matrixBHeight, col, columnInB, isMatrixBTranspose == 1);
			value += a * b;
		}
